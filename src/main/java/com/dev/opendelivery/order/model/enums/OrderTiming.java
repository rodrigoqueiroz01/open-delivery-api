...
public enum OrderTiming {
    ...
    public static OrderTiming fromString(String description) {
        return Stream.of(values()).filter(orderTiming -> orderTiming.description.equals(description)).findFirst().orElseThrow(() ->
                new IllegalArgumentException(format("O tipo '%s' não corresponde a um tipo válido. Tipos disponíveis: %s",
                        description, Stream.of(values()).map(OrderTiming::getDescription).collect(joining(", "))
                ))
        );
    }
