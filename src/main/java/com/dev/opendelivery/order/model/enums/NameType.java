...
public enum NameType {
    ...
    public static NameType fromString(String description) {
        return Stream.of(values()).filter(n -> n.description.equals(description)).findFirst().orElseThrow(() ->
                new IllegalArgumentException(format("O tipo '%s' não corresponde a um tipo válido. Tipos disponíveis: %s",
                        description, Stream.of(values()).map(NameType::getDescription).collect(Collectors.joining(", "))
                ))
        );
    }
