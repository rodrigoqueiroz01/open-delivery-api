...
public enum EventType {
    ...
    public static EventType fromString(String description) {
        return Stream.of(values()).filter(eventType -> eventType.description.equals(description)).findFirst().orElseThrow(() ->
                new IllegalArgumentException(format("O tipo '%s' não corresponde a um tipo válido. Tipos disponíveis: %s.",
                        description, Stream.of(values()).map(EventType::getDescription).collect(joining(", "))
                ))
        );
    }
