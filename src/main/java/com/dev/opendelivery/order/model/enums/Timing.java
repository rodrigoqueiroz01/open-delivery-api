...
public enum Timing {
    ...
    public static Timing fromString(String description) {
        return Stream.of(values()).filter(timing -> timing.description.equals(description)).findFirst().orElseThrow(() ->
                new IllegalArgumentException(format("O tipo '%s' não corresponde a um tipo válido. Tipos disponíveis: %s",
                        description, Stream.of(values()).map(Timing::getDescription).collect(joining(", "))
                ))
        );
    }
